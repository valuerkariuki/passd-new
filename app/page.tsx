...
export default function HomePage() {
  ...
  const stories = [
    { name: "Onesmus Njoroge, MRICS", text: "Passd.net helped me pass the Oct 2025 assessment!" },
    ...
    { name: "Godfrey Mungai, MRICS", text: "Passd.net offered great support in my direct entry." }
  ];
  ...
  return (
    <main className="min-h-screen bg-slate-50">
      <style jsx>{`
        ...
        .review-card { min-width: 280px; max-width: 280px; padding: 20px; background: white; border: 1px solid #e2e8f0; border-radius: 16px; box-shadow: 0 4px 6px -1px rgb(0 0 0 / 0.1); }
      ...
      <section id="packages" className="py-20 px-6 max-w-7xl mx-auto">
        ...
        <div className="grid md:grid-cols-4 gap-6">
          {packages.map((pkg) => (
            <div key={pkg.title} className="p-6 rounded-3xl border border-slate-200 bg-white flex flex-col">
            ...
            <div className="scrolling-text">
              {[...stories, ...stories].map((rev, i) => (
                <div key={i} className="review-card">
                  ...
                  <p className="text-xs font-bold text-[#C9A050]">— {rev.name}</p>
                </div>
              ))}
            </div>
          </div>
